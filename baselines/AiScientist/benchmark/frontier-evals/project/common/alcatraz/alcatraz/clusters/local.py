...
_MAX_UPLOAD_SIZE = 10 * 1024 * 1024 * 1024

# GPU allocation lock directory
# _GPU_LOCK_DIR = Path(tempfile.gettempdir()) / "alcatraz_gpu_locks"
_GPU_LOCK_DIR = Path.home() / ".pb" / "alcatraz_gpu_locks"

class GPUAllocator:
    """
    Manages GPU allocation with file-based locking to prevent conflicts.
    Each GPU is locked using a separate file lock, ensuring exclusive access.
    """

    def __init__(self, lock_dir: Path = _GPU_LOCK_DIR):
        self.lock_dir = lock_dir
        self.lock_dir.mkdir(parents=True, exist_ok=True)
        self._held_locks: list[UnixFileLock] = []
        self._allocated_gpu_ids: list[str] = []

    def _get_available_gpu_ids(self, timeout: float = 120.0, max_retries: int = 3, retry_delay: float = 2.0) -> list[str]:
        """Query nvidia-smi to get list of available GPU IDs with retry mechanism.
        
        Args:
            timeout: Timeout in seconds for each nvidia-smi call (default: 120s)
            max_retries: Maximum number of retry attempts (default: 3)
            retry_delay: Delay in seconds between retries (default: 2s)
        """
        import time
        ...
        for attempt in range(max_retries):
            try:
                ...
                if result.returncode != 0:
                    last_error = f"nvidia-smi failed: {result.stderr}"
                ...
                if gpu_ids:
                    return gpu_ids
                # Empty result, retry
                last_error = "nvidia-smi returned empty result"
                logger.warning(f"{last_error} (attempt {attempt + 1}/{max_retries})")
                if attempt < max_retries - 1:
                    time.sleep(retry_delay)
            except subprocess.TimeoutExpired:
                last_error = f"nvidia-smi timed out after {timeout} seconds"
                logger.warning(f"{last_error} (attempt {attempt + 1}/{max_retries})")
                if attempt < max_retries - 1:
                    time.sleep(retry_delay)
            except Exception as e:
                last_error = f"Failed to query GPUs: {e}"
                logger.warning(f"{last_error} (attempt {attempt + 1}/{max_retries})")
                if attempt < max_retries - 1:
                    time.sleep(retry_delay)
        
        logger.error(f"All {max_retries} attempts to query GPUs failed. Last error: {last_error}")
        return []
    ...
    def allocate(
        ...
        """
        Allocate specified number of GPUs with exclusive locks.
        
        Uses a retry loop to handle cases where GPUs become available after
        initial attempts. This is important for concurrent task execution.
        
        Args:
            count: Number of GPUs to allocate (-1 means all in candidate pool)
            candidate_ids: List of GPU IDs to choose from (e.g., ["2", "4", "7"]).
                          If None, all system GPUs are candidates.
                          Tasks will queue on these GPUs if all are busy.
            timeout: Total timeout in seconds to wait for GPU allocation (default: 300s = 5 min)
            retry_interval: Seconds to wait between retry attempts
            
        Returns:
            List of allocated GPU IDs (e.g., ["0", "2"])
            
        Raises:
            RuntimeError: If unable to allocate requested number of GPUs within timeout
            
        Example:
            # Only use GPUs 2, 4, 7 - tasks will queue on these 3 GPUs
            allocator.allocate(count=1, candidate_ids=["2", "4", "7"])
        """
        system_gpus = self._get_available_gpu_ids()
        if not system_gpus:
            raise RuntimeError("No GPUs available on this system")

        # Determine the candidate GPU pool
        if candidate_ids is not None:
            # Validate that candidate IDs exist in the system
            invalid_ids = set(candidate_ids) - set(system_gpus)
            if invalid_ids:
                raise RuntimeError(
                    f"Invalid GPU IDs in candidate list: {invalid_ids}. "
                    f"Available system GPUs: {system_gpus}"
                )
            available_gpus = candidate_ids
            logger.info(f"Using candidate GPU pool: {candidate_ids}")
        else:
            available_gpus = system_gpus

        target_count = len(available_gpus) if count == -1 else count
        if target_count > len(available_gpus):
            raise RuntimeError(
                f"Requested {target_count} GPUs but only {len(available_gpus)} in candidate pool"
            )
        ...
        while time.time() - start_time < timeout:
            ...
            for gpu_id in available_gpus:
                ...
                lock_file = self.lock_dir / f"gpu_{gpu_id}.lock"
                lock = UnixFileLock(str(lock_file))
                ...
                try:
                    ...
                    logger.debug(f"GPU {gpu_id} is locked by another process, skipping")
                    continue

            if len(allocated) >= target_count:
                self._held_locks = held_locks
                self._allocated_gpu_ids = allocated
                logger.info(f"Successfully allocated GPUs: {allocated} (attempt {attempt})")
                return allocated
            
            # Not enough GPUs acquired, release what we got and retry
            ...
            # Log status and wait before retrying
            remaining = timeout - (time.time() - start_time)
            logger.debug(
                f"Could only allocate {len(allocated)}/{target_count} GPUs, "
                f"retrying in {retry_interval}s (attempt {attempt}, {remaining:.1f}s remaining)"
            )
            time.sleep(retry_interval)

        raise RuntimeError(
            f"Timeout after {timeout}s: Could not allocate {target_count} GPUs. "
            f"All GPUs in pool {available_gpus} are in use by other processes."
        )

    def release(self) -> None:
        """Release all held GPU locks."""
        ...
        self._held_locks = []
        self._allocated_gpu_ids = []
        logger.info("Released all GPU locks")

    @property
    def allocated_ids(self) -> list[str]:
        """Return the list of currently allocated GPU IDs."""
        return self._allocated_gpu_ids.copy()

    def __enter__(self) -> "GPUAllocator":
        return self
...
# Shared state file for GPU load tracking (used by GPULoadBalancedAllocator)
_GPU_LOAD_STATE_FILE = Path.home() / ".pb" / "alcatraz_gpu_load_state.json"


class GPULoadBalancedAllocator:
    """
    Allocates GPUs based on current load (least loaded first).
    Tracks active tasks per GPU using a shared state file.
    
    This ensures tasks are evenly distributed across GPUs, even when tasks
    have different durations.
    
    This is useful when:
    - Tasks don't fully utilize GPU memory (e.g., H20 has much more memory than A10)
    - You want higher concurrency than available GPUs
    - You need balanced GPU utilization
    
    State file format (JSON):
        {"0": 2, "1": 1, "2": 2, "3": 0}  # GPU ID -> active task count
    """
    ...
    def __init__(self, lock_dir: Path = _GPU_LOCK_DIR):
        ...
        _GPU_LOAD_STATE_FILE.parent.mkdir(parents=True, exist_ok=True)
        self._state_lock = UnixFileLock(str(_GPU_LOAD_STATE_FILE) + ".lock")
    ...
    def _read_state(self) -> dict[str, int]:
        """Read GPU load state from file."""
        try:
            if _GPU_LOAD_STATE_FILE.exists():
                return json.loads(_GPU_LOAD_STATE_FILE.read_text())
        except (json.JSONDecodeError, IOError) as e:
            logger.warning(f"Failed to read GPU load state: {e}")
        return {}

    def _write_state(self, state: dict[str, int]) -> None:
        """Write GPU load state to file."""
        try:
            _GPU_LOAD_STATE_FILE.write_text(json.dumps(state))
        except IOError as e:
            logger.warning(f"Failed to write GPU load state: {e}")
    ...
    def allocate(
        ...
        """
        Allocate GPUs based on current load (least loaded first).
        
        Multiple tasks can be assigned to the same GPU. The allocator tracks
        active task counts and always assigns to the least loaded GPU(s).
        ...
        Returns:
            List of allocated GPU IDs (e.g., ["0"])
            
        Raises:
            RuntimeError: If no GPUs available
            
        Example:
            # 4 GPUs, tasks always go to least loaded GPU
            # If GPU loads are {0: 2, 1: 1, 2: 2, 3: 0}, next task goes to GPU 3
            allocator.allocate(count=1, candidate_ids=["0", "1", "2", "3"])
        """
        system_gpus = self._get_available_gpu_ids()
        if not system_gpus:
            raise RuntimeError("No GPUs available on this system")
        ...
        if candidate_ids is not None:
            ...
            available_gpus = candidate_ids
            logger.info(f"Using candidate GPU pool for load-balanced allocation: {candidate_ids}")
        else:
            available_gpus = system_gpus
        ...
        allocated = []
        try:
            self._state_lock.acquire(timeout=30)
            try:
                # Read current load state
                state = self._read_state()
                
                # Initialize missing GPUs with 0 load
                for gpu_id in available_gpus:
                    if gpu_id not in state:
                        state[gpu_id] = 0

                # Select GPUs with lowest load
                for _ in range(target_count):
                    # Sort by load (ascending), then by GPU ID for determinism
                    sorted_gpus = sorted(
                        [(gpu_id, state.get(gpu_id, 0)) for gpu_id in available_gpus],
                        key=lambda x: (x[1], x[0])
                    )
                    # Pick the least loaded GPU
                    best_gpu = sorted_gpus[0][0]
                    allocated.append(best_gpu)
                    # Increment load for this GPU (for subsequent selections in same call)
                    state[best_gpu] = state.get(best_gpu, 0) + 1

                # Write updated state
                self._write_state(state)
                
                logger.info(
                    f"Load-balanced allocated GPUs: {allocated} "
                    f"(current loads: {{{', '.join(f'{k}: {v}' for k, v in sorted(state.items()))}}})"
                )
            finally:
                self._state_lock.release()
        except LockTimeout:
            # Fallback to random selection if lock times out
            logger.warning("State lock timeout, falling back to random selection")
            allocated = random.sample(available_gpus, target_count)

        self._allocated_gpu_ids = allocated
        return allocated

    def release(self) -> None:
        """Release allocated GPUs by decrementing their load counts."""
        if not self._allocated_gpu_ids:
            return
        ...
        try:
            ...
            try:
                ...
                for gpu_id in self._allocated_gpu_ids:
                    if gpu_id in state:
                        state[gpu_id] = max(0, state[gpu_id] - 1)
                
                self._write_state(state)
                logger.info(
                    f"Released GPUs: {self._allocated_gpu_ids} "
                    f"(current loads: {{{', '.join(f'{k}: {v}' for k, v in sorted(state.items()))}}})"
                )
            finally:
                self._state_lock.release()
        except LockTimeout:
            logger.warning(f"State lock timeout during release for GPUs: {self._allocated_gpu_ids}")
    ...
    def __enter__(self) -> "GPULoadBalancedAllocator":
        return self
...
class ClusterConfig(ABC, SerializableBaseModel):
    ...
    gpu_count: int = -1  # Number of GPUs to allocate (-1 means all GPUs)
    gpu_device_ids: list[str] | None = None  # Specific GPU IDs to use (e.g., ["0", "1"]), overrides gpu_count
    gpu_candidate_ids: list[str] | None = None  # Candidate GPU pool (e.g., ["2", "4", "7"]), tasks queue on these
    gpu_auto_allocate: bool = True  # Auto-allocate GPUs with file locks (only used when gpu_share_mode=False)
    gpu_share_mode: bool = True  # Share mode (default): multiple tasks share GPUs via load balancing
...
class LocalConfig(ClusterConfig):
    ...
    def build(self) -> "LocalCluster":
        return LocalCluster(
            ...
            gpu_candidate_ids=self.gpu_candidate_ids,
            gpu_auto_allocate=self.gpu_auto_allocate,
            gpu_share_mode=self.gpu_share_mode,
...
class BaseAlcatrazCluster(ABC):
    ...
    def __init__(
        ...
        gpu_count: int = -1,  # Number of GPUs to allocate (-1 means all GPUs)
        gpu_device_ids: list[str] | None = None,  # Specific GPU IDs to use
        gpu_candidate_ids: list[str] | None = None,  # Candidate GPU pool for auto-allocation
        gpu_auto_allocate: bool = True,  # Auto-allocate GPUs with file locks (only when gpu_share_mode=False)
        gpu_share_mode: bool = True,  # Share mode (default): multiple tasks share GPUs via load balancing
        ...
        self.gpu_candidate_ids = gpu_candidate_ids
        self.gpu_auto_allocate = gpu_auto_allocate
        self.gpu_share_mode = gpu_share_mode
        self._gpu_allocator: GPUAllocator | GPULoadBalancedAllocator | None = None
    ...
    async def _start(self) -> None:
        ...
        if self.no_network:
            ...
            try:
                ...
                raise
            if isinstance(self, LocalCluster):
                async def _safe_remove_network(network):
                    try:
                        await asyncio.to_thread(network.remove)
                    except Exception as e:
                        if "NotFound" in type(e).__name__ or "404" in str(e):
                            logger.debug("Network already removed, ignoring: %s", e)
                        else:
                            raise
                self._exit_stack.push_async_callback(
                    lambda: _safe_remove_network(self.docker_network)
                )
        ...
        if self.is_nvidia_gpu_env:
            if self.gpu_device_ids is not None:
                # Use explicitly specified GPU IDs (highest priority)
                device_requests = [
                ...
                logger.info(f"Using specified GPU device IDs: {self.gpu_device_ids}")
            elif self.gpu_share_mode:
                # Share mode: use load-balanced allocation without exclusive locks
                # Multiple tasks can share the same GPU, always picks least loaded
                self._gpu_allocator = GPULoadBalancedAllocator()
                allocated_ids = self._gpu_allocator.allocate(
                    count=self.gpu_count,
                    candidate_ids=self.gpu_candidate_ids,
                )
                device_requests = [
                    docker.types.DeviceRequest(
                        device_ids=allocated_ids,
                        capabilities=[["gpu"]]
                    )
                ]
                logger.info(f"Load-balanced allocated GPU device IDs: {allocated_ids} (share mode)")
            elif self.gpu_auto_allocate:
                # Exclusive mode: auto-allocate GPUs with file locks to prevent conflicts
                self._gpu_allocator = GPUAllocator()
                # Let RuntimeError propagate - do NOT fall back to count-based allocation
                # as that can cause GPU conflicts between concurrent containers
                allocated_ids = self._gpu_allocator.allocate(
                    count=self.gpu_count,
                    candidate_ids=self.gpu_candidate_ids,  # Only allocate from candidate pool
                )
                device_requests = [
                    docker.types.DeviceRequest(
                        device_ids=allocated_ids,
                ...
                logger.info(f"Auto-allocated GPU device IDs: {allocated_ids} (exclusive mode)")
            else:
                # Use count-based allocation (original behavior, may have conflicts)
                device_requests = [
                    docker.types.DeviceRequest(
                        count=self.gpu_count,
                        capabilities=[["gpu"]]
                ...
                logger.info(f"Using count-based GPU allocation: {self.gpu_count}")
        ...
        for i, image in enumerate(images):
            ...
            _, ctr = await self._create_container(
                ...
                #     else []
                # ),
                device_requests=device_requests if i == 0 else [],
                stdin_open=True,
                tty=True,
    ...
    async def _stop(self) -> None:
        ...
        # Release GPU locks if any were acquired
        if self._gpu_allocator is not None:
            self._gpu_allocator.release()
    ...
    async def create_kernel_on_machine(
        ...
        # When using local_network (--network host), container ports are directly
        # accessible on the host, so we don't need socat for port forwarding
        if self.local_network:
            logger.info("Using local_network=True (--network host), skipping socat port forwarding")
            # Ports are already accessible on localhost, just update connection_file IP
    ...
    async def _start_docker_compose(self) -> None:
        ...
        for network in new_networks:
            if isinstance(self, LocalCluster):
                async def _safe_remove_network_compose(net):
                    try:
                        ...
                        if "NotFound" in type(e).__name__ or "404" in str(e):
                            ...
                            raise
                self._exit_stack.push_async_callback(
                    lambda network=network: _safe_remove_network_compose(network)  # type: ignore
                )
...
class LocalCluster(BaseAlcatrazCluster):
    def __init__(
        ...
        super().__init__(
            ...
            gpu_candidate_ids=gpu_candidate_ids,
            gpu_auto_allocate=gpu_auto_allocate,
