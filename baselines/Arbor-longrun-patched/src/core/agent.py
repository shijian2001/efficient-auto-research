        ...
        for turn in range(1, self.config.max_turns + 1):
            ...
            if not tool_calls:
                ...
                if (
                    ...
                    continue
                if self.config.continue_until_budget and turn < self.config.max_turns:
                    budget_continue_nudges += 1
                    _print_status(
                        "Text-only completion before the budget cutoff; "
                        f"nudging coordinator to continue (nudge {budget_continue_nudges})."
                    )
                    self.messages.append({
                        "role": "user",
                        "_internal": "budget_continue_nudge",
                        "content": (
                            "Do not end the research run yet. You returned text without calling a tool. "
                            "Continue the public-data research loop now: inspect the current tree, propose or "
                            "dispatch another concrete experiment, and use the available tools. Only enter "
                            "finalization when the run reaches its final budget window."
                        ),
                    })
                    continue
