...
def _child_env(*, scrub: bool = False) -> dict[str, str]:
    """Return a subprocess environment, optionally without controller state."""
...
_BLOCKED_PATTERNS: list[tuple[re.Pattern, str]] = [
    ...
    (re.compile(r"(?:^|[/\\])mlebench[/\\]data[/\\][^/\\]+[/\\](?:prepared[/\\])?private(?:[/\\]|$)", re.IGNORECASE), "Access to private test data is blocked"),
]
...
class BashTool(Tool):
    ...
    def __init__(
        ...
        *,
        ...
        scrub_controller_env: bool = False,
        **kwargs: Any,
    ):
        super().__init__(cwd=cwd, **kwargs)
        self.scrub_controller_env = scrub_controller_env
    ...
    async def _run_foreground(self, command: str, timeout: int) -> str:
        ...
        try:
            proc = await asyncio.create_subprocess_shell(
                ...
                cwd=self.cwd,
                env=_child_env(scrub=self.scrub_controller_env),
