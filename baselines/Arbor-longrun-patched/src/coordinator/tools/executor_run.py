...
def _guard_protected(config: Any, tree: Any, worktree_path: Path) -> tuple[list[str], dict[str, str]]:
    ...
    try:
        ...
        raise RuntimeError(f"protected-path guard failed closed: {exc}") from exc
...
def _check_tamper(
    ...
    try:
        ...
        log.warning("integrity: verify failed closed: %s", exc)
        return [type("ProtectedChangeFailure", (), {"path": "<protected-tree>", "kind": "modified"})()]
    if changes:
...
def _validate_dispatch(
    ...
    if tree.meta.get("achieved_medal") == "gold" and not tree.meta.get("continue_until_budget", False):
        return None, 0, (
