...
def get_coordinator_tools(
    ...
    # Long-run MLE mode retains warnings/interventions but never emits a hard
    # convergence stop before the orchestrator's time-budget cutoff.
    if getattr(config, "continue_until_budget", False):
        conv_config = ConvergenceConfig.from_dict({
            **conv_config.model_dump(),
            "stop_after": 10**9,
        })
