...
class CoordinatorOrchestrator:
    ...
    def _apply_eval_contract(self) -> None:
        ...
        for key in (
            ...
            if key in plugin.eval_contract and plugin.eval_contract[key] is not None:
                # The plugin contract is the authoritative, frozen protocol for
                # this run. In particular, it must override IdeaTree defaults
                # such as metric_direction=maximize and
                # test_semantics=independent_test.
                self.tree.meta[key] = plugin.eval_contract[key]
        self.tree.meta["continue_until_budget"] = bool(
            getattr(self.config, "continue_until_budget", False)
        )
        self.tree.save()
    ...
    def _create_coordinator(self) -> Agent:
        ...
        agent_config = AgentConfig(
            ...
            continue_until_budget=self.config.continue_until_budget,
            auto_git=False,
    ...
    def _eval_info_parts(self, header: str) -> list[str]:
        ...
        if self.tree.meta.get("eval_cmd_test"):
            if self.tree.meta.get("test_semantics") == "artifact_verification_only":
                lines.append(
                    "- Artifact verification (not B_test/official grading): "
                    f"`{self.tree.meta['eval_cmd_test']}`"
                )
            else:
    ...
    def _build_initial_prompt(self) -> str:
        ...
        if baseline is not None:
            ...
            parts.append(
                ...
                "2. Find the configured evaluation and verification protocol\n"
        ...
        merge_verification = (
            "artifact integrity only; official/test grading remains external"
            if self.tree.meta.get("test_semantics") == "artifact_verification_only"
            else "confirmed on B_test before merging"
        )
        parts.append(
            f"## Guidelines\n\n"
