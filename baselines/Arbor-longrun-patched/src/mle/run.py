...
def _sanitize_run_id(value: str) -> str:
    ...
    return component or "mle-run"


def _trunk_branch_for_run(run_id: str) -> str:
    return f"arbor/mle/{_sanitize_run_id(run_id)}/trunk"
...
def main(argv: list[str] | None = None) -> int:
    ...
    run_id = _sanitize_run_id(args.run_name or f"mle_{args.competition_id}")
    ...
    completed = subprocess.run(command, cwd=spec.workspace, env={
        **os.environ,
        "ARBOR_PYTHON": sys.executable,
    })
    try:
