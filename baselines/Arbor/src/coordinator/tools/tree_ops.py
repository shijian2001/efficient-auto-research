...
class TreeSetMetaTool(Tool):
    ...
    input_schema: dict[str, Any] = {
        ...
        "properties": {
            ...
            "test_semantics": {
                "type": "string",
                "enum": ["independent_test", "artifact_verification_only"],
                "description": "Whether eval_cmd_test produces an independent score or only verifies a bound artifact.",
            },
            "eval_timeout": {
                "type": "integer",
    ...
    async def execute(self, **kwargs: Any) -> str:
        ...
        # String fields
        for key in ("eval_cmd", "eval_cmd_test", "test_semantics", "dataset_info", "metric_direction", "submission_path", "sample_submission_path"):
            if key in kwargs and kwargs[key] is not None:
                self._tree.meta[key] = kwargs[key]
                updated.append(f"{key} = {kwargs[key]}")
    ...
    def _write_baseline_cache(self) -> None:
        ...
        cache = {
            ...
            "test_semantics": meta.get("test_semantics"),
